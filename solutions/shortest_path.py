...
class Graph:
    ...
    def dijkstra(self, begin_id: str, end_id: str) -> Tuple[List[str], float]:
        """
        Perform a dijkstra search beginning at vertex with id begin_id and terminating at vertex with id end_id
        :param begin_id: id of starting vertex
        :param end_id: id of destination vertex
        :return: tuple of path and cost needed
        """
        if begin_id not in self.vertices or end_id not in self.vertices:
            return [], 0

        mapping, cloud, pq = {}, {}, PriorityQueue()

        for vertex in self.vertices:
            if vertex == begin_id:
                mapping[vertex] = 0
            else:
                mapping[vertex] = float('inf')
            pq.push(mapping[vertex], self.vertices[vertex])

        while not pq.empty():
            wgt, point = pq.pop()
            for edge, weight in point.adj.items():
                if mapping[point.id] + weight < mapping[edge]:
                    mapping[edge] = weight + wgt
                    pq.update(mapping[edge], self.vertices[edge])
                    cloud[edge] = point.id
        if not cloud or len(cloud) == 0:
            return [], 0
        return self.build_path(cloud, begin_id, end_id)

    def a_star(self, begin_id: str, end_id: str,
               metric: Callable[[Vertex, Vertex], float]) -> Tuple[List[str], float]:
        """
        Perform a A* search beginning at vertex with id begin_id and terminating at vertex with id end_id
        :param begin_id: id of starting vertex
        :param end_id: id of destination vertex
        :param metric: callable function that takes starting and destination vertex id for arguments and returns
        the distance
        :return: tuple of path and cost needed
        """
        if begin_id not in self.vertices or end_id not in self.vertices:
            return [], 0
        if end_id in self.vertices[begin_id].adj.keys():
        ...
        while not pq.empty():
            ...
            for edge, weight in point.adj.items():
                if mapping[point.id] + weight < mapping[edge]:
                    metric_val = metric(self.vertices[edge], self.vertices[end_id])
                    mapping[edge] = mapping[point.id] + weight
                    cloud[edge] = point.id
                    if edge in pq.locator:
                        pq.update(mapping[edge] + metric_val, self.vertices[edge])
                    else:
                        pq.push(mapping[edge] + metric_val, self.vertices[edge])
        try:
            return self.build_path(cloud, begin_id, end_id)
        except TypeError:
            return [], 0
